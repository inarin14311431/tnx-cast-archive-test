...
async function startComparison(button){
  ...
  try{
    ...
    location.reload();
  }catch(error){console.error(error);sessionStorage.removeItem(SESSION_KEY);hideBusy();button.disabled=false;alert(`差分比較に失敗しました：${error?.message||error}`);}
}
...
async function restoreComparison(){
  ...
  const publicId=new URLSearchParams(location.search).get("id")||"";if(!publicId)return clearSession();
  const {data:auth}=await supabase.auth.getUser();if(!auth?.user)return clearSession();
  const archiveBundle=await loadSheetBundle({publicId,ownerId:auth.user.id});
  showComparisonModal({...stored,archiveBundle,differences:compareBundles(archiveBundle,stored.warehouseBundle)});
}
...
async function fetchCharacterSheetPayload(sourceUrl){
  ...
  const encoded=encodeURIComponent(key);const urls=[primary,`https://character-sheets.appspot.com/tnx/display.html?ajax=1&key=${encoded}`,`https://character-sheets.appspot.com/tnx/display?key=${encoded}&ajax=1`,`https://character-sheets.appspot.com/tnx/display.html?key=${encoded}&ajax=1`];
...
function compareBundles(archive,warehouse){const left=comparableBundle(archive),right=comparableBundle(warehouse),out=[];for(const category of Object.keys(CATEGORY_LABELS))diffObject(left[category]||{},right[category]||{},category,[],out);return out;}
function comparableBundle(bundle={}){const c=bundle.character||{},pick=fields=>Object.fromEntries(fields.map(field=>[field,scalar(c[field])])),skills=Array.isArray(bundle.skills)?bundle.skills:[];return{basic:pick(["character_name","character_kana","handle","handle_kana","player_name","affiliation","citizen_rank","summary","profile"]),personal:pick(["age","gender","height","weight","eyes","hair","skin","life_path_origin","life_path_experience","life_path_encounter"]),styles:pick(["style_1","style_1_mark","style_1_attribute","style_2","style_2_mark","style_2_attribute","style_3","style_3_mark","style_3_attribute"]),abilities:pick(["reason_base","reason_gear","reason_control_base","reason_control_gear","passion_base","passion_gear","passion_control_base","passion_control_gear","life_base","life_gear","life_control_base","life_control_gear","mundane_base","mundane_gear","mundane_control_base","mundane_control_gear","cs_base","cs_gear"]),general:rowsByIdentity(skills.filter(r=>r.category==="general")),social:rowsByIdentity(skills.filter(r=>r.category==="social")),connection:rowsByIdentity(skills.filter(r=>r.category==="connection")),styleSkills:rowsByIdentity(skills.filter(r=>r.category==="style")),outfits:rowsByIdentity(Array.isArray(bundle.outfits)?bundle.outfits:[],r=>`${r.category||"other"}:${r.name||"名称なし"}`)};}
function rowsByIdentity(rows,identity=r=>r.description===STYLE_SEPARATOR_MARKER?`区切り:${r.name}`:r.name||"名称なし"){const out={},counts=new Map();for(const row of rows){const base=String(identity(row)),count=(counts.get(base)||0)+1;counts.set(base,count);const key=count===1?base:`${base} #${count}`,clean={};for(const [field,value] of Object.entries(row||{})){if(["id","character_id","created_at","updated_at","sort_order","_key","ofc_details"].includes(field))continue;clean[field]=scalar(value);}out[key]=clean;}return out;}
function scalar(value){if(value===null||value===undefined)return"";if(typeof value==="boolean"||typeof value==="number")return value;if(typeof value==="object")return JSON.stringify(value);return String(value).trim();}
...
function renderDifference(item){return`<article><strong>${esc(CATEGORY_LABELS[item.category]||item.category)} / ${esc(item.path)}</strong><div><span><b>CAST ARCHIVE</b>${esc(displayValue(item.archive))}</span><span><b>倉庫</b>${esc(displayValue(item.warehouse))}</span></div></article>`;}
async function copyDifferences(context){const lines=["キャラクターシート倉庫との差分",`比較日時: ${formatDate(context.comparedAt)}`,`URL: ${context.sourceUrl}`,`差分: ${context.differences.length}件`,""];if(!context.differences.length)lines.push("差分なし");for(const item of context.differences)lines.push(`[${CATEGORY_LABELS[item.category]||item.category}] ${item.path}`,`CAST ARCHIVE: ${displayValue(item.archive)}`,`倉庫: ${displayValue(item.warehouse)}`,"");const text=lines.join("\n");try{await navigator.clipboard.writeText(text);setMessage("差分をクリップボードへコピーしました。","saved");}catch{prompt("差分をコピーしてください。",text);}}
async function adoptWarehouse(context,dialog){if(!confirm("現在のCAST ARCHIVEをスナップショットに保存し、比較したキャラクターシート倉庫版を編集画面へ反映します。続行しますか？"))return;disableChoices(dialog,true);try{const snapshots=await waitForSnapshots();setMessage("CAST ARCHIVE版をスナップショットへ保存しています…");await snapshots.createCurrent(`比較前 CAST ARCHIVE ${formatDate(context.comparedAt)}`);setMessage("キャラクターシート倉庫版を編集画面へ反映しています…");await applyLegacyPayload(context.externalPayload);const target=document.querySelector("#character-sheet-url");if(target){target.value=context.sourceUrl;target.dispatchEvent(new Event("input",{bubbles:true}));target.dispatchEvent(new Event("change",{bubbles:true}));}clearSession();dialog.close("adopted");}catch(error){console.error(error);setMessage(`処理に失敗しました：${error?.message||error}`,"error");disableChoices(dialog,false);}}
async function keepArchive(context,dialog){if(!confirm("比較したキャラクターシート倉庫版をスナップショットに保存し、現在のCAST ARCHIVE版を編集画面に残します。続行しますか？"))return;disableChoices(dialog,true);try{const snapshots=await waitForSnapshots(),snapshotData={character:{...context.archiveBundle.character,...context.warehouseBundle.character,character_sheet_url:context.sourceUrl},skills:context.warehouseBundle.skills,outfits:context.warehouseBundle.outfits};setMessage("キャラクターシート倉庫版をスナップショットへ保存しています…");await snapshots.createBundle(snapshotData,`キャラクターシート倉庫 ${formatDate(context.comparedAt)}`);clearSession();dialog.close("kept-archive");}catch(error){console.error(error);setMessage(`スナップショット作成に失敗しました：${error?.message||error}`,"error");disableChoices(dialog,false);}}
...
function showBusy(){hideBusy();const overlay=document.createElement("div");overlay.id="character-sheet-compare-busy";overlay.className="character-sheet-compare-busy";overlay.innerHTML="<div><strong>キャラクターシート倉庫を取得して比較しています…</strong><small>比較用に倉庫データを変換しています。画面は自動的に元へ戻ります。</small></div>";document.body.append(overlay);}
function hideBusy(){document.querySelector("#character-sheet-compare-busy")?.remove();}
function displayValue(value){if(value===""||value===null||value===undefined)return"（空欄）";if(typeof value==="boolean")return value?"あり":"なし";return String(value);}
