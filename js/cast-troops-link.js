...
async function initialize() {
  ...
  if (primary && !primary.querySelector("[data-cast-troops-jump]")) {
    const link = document.createElement("a"); link.href="#cast-troops-panel"; link.className="cast-edit-link cast-troops-jump"; link.dataset.castTroopsJump="1"; link.innerHTML=`<span>トループ ${troops.length}</span><small>TROOPS</small>`; primary.append(link);
  }
  const panel=document.createElement("section"); panel.id="cast-troops-panel"; panel.className="cast-troops-panel";
  ...
  const mobile=document.querySelector("#mobile-cast-view"); (mobile||document.querySelector("#cast-content"))?.before(panel);
  const roots=[document.querySelector("#mobile-cast-view"),document.querySelector("#quick-sheet-pages")].filter(Boolean);
  roots.forEach(root=>new MutationObserver(()=>decorateExperience(expText)).observe(root,{childList:true,subtree:true}));
}
function decorateExperience(expText){
  const desktop=document.querySelector("#cast-exp");if(desktop)desktop.textContent=`${expText} EXP`;
  document.querySelectorAll(".mobile-cast-meta div").forEach(item=>{if(item.querySelector("dt")?.textContent?.trim()==="EXP"){const dd=item.querySelector("dd");if(dd)dd.textContent=expText;}});
  document.querySelectorAll("#quick-sheet-pages .quick-sheet__identity-meta div").forEach(item=>{if(item.querySelector("dt")?.textContent?.trim()==="EXP"){const dd=item.querySelector("dd");if(dd)dd.textContent=expText;}});
}
