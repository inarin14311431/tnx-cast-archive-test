...
function render(){
  ...
  list.innerHTML=filtered.map(t=>{const linked=characters.find(c=>c.id===t.character_id);return `<article class="troop-card"><div class="troop-card__head"><div><p>${escapeHtml(t.public_id)}</p><h2>${escapeHtml(t.name||"名称未設定")}</h2></div><span class="troop-visibility troop-visibility--${escapeHtml(t.visibility)}">${t.visibility==="public"?"公開":"非公開"}</span></div><div class="troop-card__stats"><span>LEVEL / CS <strong>${t.level}</strong></span><span>MAX MEMBERS <strong>${t.member_max}</strong></span><span>EXP <strong>${t.experience_spent??0}</strong></span><span>STYLE <strong class="troop-card__style">${escapeHtml(t.style_1||"—")}</strong></span></div><p class="troop-card__linkage">${linked?`所属キャスト：${escapeHtml(linked.character_name)}`:"所属キャスト：未設定"}</p><div class="troop-card__actions"><a href="./troop.html?id=${encodeURIComponent(t.public_id)}">閲覧 <small>OPEN</small></a><a href="./troop.html?id=${encodeURIComponent(t.public_id)}&edit=1">編集 <small>EDIT</small></a></div></article>`;}).join("");
}
