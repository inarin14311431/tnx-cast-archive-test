...
export function installLegacyOutfitCompatibility({
  ...
  const handleLoaded = event => {
    ...
    const finish = () => {
      ...
      finished = true;
      cleanup();
      if (!userEdited) markSaved();
    };

    const scheduleFinish = () => {
      if (finished || userEdited) return;
      if (quietTimer) globalThis.clearTimeout?.(quietTimer);
      quietTimer = globalThis.setTimeout?.(finish, QUIET_MS);
    };
    ...
    if (typeof Observer === "function") {
      observer = new Observer(scheduleFinish);
      observer.observe(outfitRoot, { childList: true, subtree: true, attributes: true });
    }
  ...
  return () => {
    cleanupActive();
    windowRef.removeEventListener(LOAD_EVENT, handleLoaded);
  };
}
