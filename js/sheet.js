...
const STRUCTURED_FIELDS = [
  ...
  ["hair", "#hair"],
  ["skin", "#skin"],
  ["life_path_origin", "#life-path-origin"],
  ["life_path_experience", "#life-path-experience"],
  ["life_path_encounter", "#life-path-encounter"]
];
...
function generalColumnCounts() {
  ...
  return skills
    .filter(item => item.category === "general" && item._slotColumn)
    .reduce((counts, item) => {
...
function addGeneralSkill() {
  ...
  requestAnimationFrame(() => {
    document.querySelector(`#general-skills tr[data-skill-key="${skill._key}"] [data-f="name"]`)?.focus();
  });
}
...
function createNew() {
  ...
  skills = GENERAL_MASTER
    .filter(item => item[2] === "general")
    .map(([name, suit]) => ({
      ...blankSkill("general"),
      name,
      level: 1,
...
function fillCharacter(data) {
  ["character_name", "character_kana", "handle", "player_name", "affiliation", "citizen_rank", "summary", "profile"]
    .forEach(name => {
      const element = $("#" + name.replaceAll("_", "-"));
      if (element) element.value = data[name] ?? "";
    });

  for (const [name, selector] of STRUCTURED_FIELDS) {
...
function renderStyles() {
  const options = '<option value="">選択</option>' +
    STYLE_DATA.map(item => `<option>${esc(item.name)}</option>`).join("");
  const attributes = '<option value="">属性を選択</option>' +
    UTSUWA_ATTRIBUTES.map(item => `<option>${esc(item.name)}</option>`).join("");
  ...
  $("#style-grid").innerHTML = [1, 2, 3].map(i => `
    <article class="style-card">
      ...
      <section class="divine-field"><ruby><strong id="divine-${i}">未選択</strong><rt id="divine-${i}-yomi"></rt></ruby><span>神業</span></section>
    </article>
  `).join("");
...
function styleRecord(i) {
  ...
  return name === "ウツワ"
    ? UTSUWA_ATTRIBUTES.find(item => item.name === $(`#style-${i}-attribute`).value) || null
    : STYLE_DATA.find(item => item.name === name) || null;
}
...
function updateDivines(apply) {
  ...
  $("#style-warning").textContent =
    [1, 2, 3].filter(i => $(`#style-${i}`).value).length === 3
      ? ""
      : "3枠すべてのスタイルを選択してください。";

  if (!apply || loading) return;

  const old = { ...styleBaseline };
  calculateBaselines();
  for (const [key] of ABILITIES) {
...
function adjustBaseline(id, oldBase, newBase) {
  ...
  if (element && (Number(element.value || 0) === oldBase || Number(element.value || 0) === 0)) {
    element.value = newBase;
  }
}
...
function normalizeSkill(skill) {
  const result = {
    ...blankSkill(skill.category),
    ...skill,
    _key: skill.id || crypto.randomUUID(),
    level: Number(skill.level || 0),
    free_level: Number(skill.free_level || 0),
    skill_kind: skill.skill_kind || inferKind(skill)
  };
...
function inferKind(skill) {
  if (skill.category === "style") {
    return window.TNXStyleSkillKinds?.fromLabel(skill.type || skill.kind) ||
      (/演出|方向/.test(skill.type || "") ? "direction"
        : /奥義/.test(skill.type || "") ? "ultimate"
          : /秘技/.test(skill.type || "") ? "secret"
            : /なし/i.test(skill.type || "") ? "none"
              : "normal");
  }
  ...
  for (const [name, , kind] of GENERAL_MASTER) {
    ...
    if (skill) {
      for (const duplicate of matches) {
        ...
        SUITS.forEach(suit => {
          skill[suit] = Boolean(skill[suit] || duplicate[suit]);
        });
...
function mergedGeneral() {
  ...
  return output.sort((a, b) => {
    const ai = GENERAL_MASTER.findIndex(item => item[0] === a.name);
    const bi = GENERAL_MASTER.findIndex(item => item[0] === b.name);
  ...
  return `
    <section class="skill-group ${esc(category)}" data-skill-category="${esc(category.split(" ")[0])}">
      ...
      <table class="skill-table ${detail ? "has-detail" : "no-detail"}">
        ...
        <tbody>${rows.map(item => skillRow(item, detail)).join("")}</tbody>
      </table>
    </section>`;
}
...
function skillRow(skill, detail) {
  ...
  const labels = {
    ...
    ultimate: "奥義",
    direction: "演出",
    ...(window.TNXStyleSkillKinds?.labels || {})
  };
  const slotAttribute = skill._blankSlot
  ...
  $$("[data-skill-key]").forEach(row => {
    row.querySelectorAll("[data-f]").forEach(element => {
      element.oninput = () => {
        ...
        skill[field] = element.type === "checkbox"
          ? element.checked
          : element.type === "number"
        ...
        if (SUITS.includes(field)) {
          ...
          skill.level = level;
          skill.free_level = Math.min(Math.max(Number(skill.free_level || 0), 0), level);
...
function normalizeOutfit(outfit) {
  return {
    ...blankOutfit(),
    ...outfit,
    _key: outfit.id || crypto.randomUUID(),
...
function outfitFields(outfit) {
  ...
  if (outfit.category === "residence") {
    return common + `<label>外界<input data-o="mundane_modifier" type="number" value="${outfit.mundane_modifier}"></label><label>部位／エリア<input data-o="slot" value="${esc(outfit.slot)}"></label>` + description;
  }
...
function renderOutfits() {
  $("#outfit-list").innerHTML = outfits.map(outfit => `
    <article class="outfit-card outfit-form" data-outfit-key="${outfit._key}">
      <header>
        <label>分類<select data-o="category">${Object.entries(OUTFIT_LABELS).map(([value, label]) => `<option value="${value}" ${outfit.category === value ? "selected" : ""}>${label}</option>`).join("")}</select></label>
        <button class="row-delete" data-delete-outfit="${outfit._key}" type="button">×</button>
      </header>
      <div class="outfit-fields">${outfitFields(outfit)}</div>
    </article>
  `).join("") || "<p>アウトフィット未登録</p>";
  ...
  $$("[data-outfit-key]").forEach(card => {
    card.querySelectorAll("[data-o]").forEach(element => {
      element.oninput = () => {
        ...
        if (element.dataset.o === "category") renderOutfits();
        recalc();
        markDirty();
      };
    });
...
function current(id) {
  return Number($(`#${id}-base`)?.value || 0);
}

function final(id) {
  return current(id) + Number($(`#${id}-mod`)?.value || 0);
}
...
function collectCharacter() {
  ...
  const payload = {
    character_name: $("#character-name").value.trim(),
    character_kana: $("#character-kana").value.trim(),
    handle: $("#handle").value.trim(),
    player_name: $("#player-name").value.trim(),
    affiliation: $("#affiliation").value.trim(),
    citizen_rank: $("#citizen-rank").value.trim(),
  ...
  for (const [name, selector] of STRUCTURED_FIELDS) {
    const element = $(selector);
    payload[name] = element ? element.value.trim() : "";
  }
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    payload[`style_${i}_attribute`] = $(`#style-${i}-attribute`)?.value || "";
    payload[`divine_${i}`] = style?.divine || "";
    payload[`divine_${i}_yomi`] = style?.divineYomi || style?.divine || "";
  }

  for (const [key] of ABILITIES) {
    payload[`${key}_base`] = current(key);
    payload[`${key}_growth`] = Math.max(0, current(key) - Number(styleBaseline[key] || 0));
    payload[`${key}_gear`] = Number($(`#${key}-mod`).value || 0);
    payload[`${key}_manual`] = 0;
    payload[`${key}_value`] = final(key);

    const controlKey = `${key}-control`;
    payload[`${key}_control_base`] = current(controlKey);
    payload[`${key}_control_growth`] = Math.max(0, current(controlKey) - Number(styleBaseline[controlKey] || 0));
    payload[`${key}_control_gear`] = Number($(`#${controlKey}-mod`).value || 0);
    payload[`${key}_control_manual`] = 0;
    payload[`${key}_control`] = final(controlKey);
  }
...
function collectSkills() {
  return skills
    .filter(item => Number(item.level) > 0 && item.name.trim())
    .map((item, index) => ({
      category: item.category,
      name: item.name,
      level: Number(item.level || 0),
      free_level: Math.min(
        Math.max(Number(item.free_level || 0), 0),
...
function collectOutfits() {
  return outfits
    ...
    .map((item, index) => ({
      ...
      slot: item.slot || "",
      description: item.description || "",
      control_modifier: Number(item.control_modifier || 0),
      cs_modifier: Number(item.cs_modifier || 0),
      mundane_modifier: Number(item.mundane_modifier || 0),
      sort_order: index
    }));
}
...
async function saveAll(force) {
  ...
  try {
    ...
    console.error(error);
    dirty = true;
    setStatus(jpError(error?.message), "error");
    pulse("error");
  } finally {
    saving = false;
    if (pending) {
...
function openImport(mode) {
  importMode = mode;
  $("#tsv-title").textContent = `${mode.toUpperCase()} TSV取込`;
  $("#tsv-text").value = "";
  ...
  if (importMode === "skd") {
    for (const row of rows) {
      skills.push({
        ...blankSkill("style"),
        name: row["名称"] || "",
        skill_kind: window.TNXStyleSkillKinds?.fromLabel(row["種別"]) ||
          (/奥義/.test(row["種別"] || "") ? "ultimate" : /秘技/.test(row["種別"] || "") ? "secret" : "normal"),
...
function jpError(message = "") {
  if (/save_character_bundle|PGRST202|Could not find the function/i.test(message)) {
    return "安全保存機能が未設定です。Supabaseで supabase/10_transactional_character_save.sql を実行してください。";
  }
  if (/characters_visibility_check/i.test(message)) {
    return "公開状態を保存できません。Supabaseの公開状態制約を更新してください。";
  }
  if (/row-level security|RLS|42501/i.test(message)) {
    return "保存権限がありません。ログイン状態を確認してください。";
  }
  if (/schema cache/i.test(message)) {
    return "データベース項目を確認できません。Supabaseのスキーマを再読み込みしてください。";
  }
  if (/network|fetch/i.test(message)) {
    return "通信に失敗しました。既存データは変更されていません。ネットワーク接続を確認してください。";
  }
