(() => {
  ...
  function install() {
    ...
    if (!button) {
      ...
      button.innerHTML = "<span>HELP</span><small>TRANSFER GUIDE</small>";
    ...
    const parent = tsv.parentElement;
    ...
    dialog.innerHTML = `
      <div class="transfer-help-shell">
        ...
        <div class="transfer-help-body">
          ...
          <section class="transfer-help-block transfer-help-block--setup"><span class="transfer-help-step">初回のみ</span><h3>転記BMをブラウザへ登録</h3><ol><li><b>「転記BM」</b>を押してブックマークレットをコピーします。</li><li>ブラウザで適当なページをブックマーク登録し、ブックマークの編集画面を開きます。</li><li>ブックマークのURL欄をすべて消し、コピーした <code>javascript:...</code> を貼り付けて保存します。</li><li>名前は「N◎VA転記」など、分かりやすいものにしておくと便利です。</li></ol><p class="transfer-help-note">転記BMは一度登録すれば、通常はキャストごとに作り直す必要はありません。</p></section>
          <section class="transfer-help-block transfer-help-block--use"><span class="transfer-help-step">転記するたび</span><h3>転記TSV → 転記BMの順に実行</h3><ol><li>このキャスト画面で<b>「転記TSV」</b>を押します。現在のキャストデータがクリップボードへコピーされます。</li><li>転記先の<b>キャラクターシート編集画面</b>を開きます。</li><li>先ほど登録した<b>「N◎VA転記」ブックマーク</b>を実行します。</li><li>転記処理が始まり、基本情報・スタイル・能力値／制御値・技能・アウトフィットが入力されます。</li><li>転記後は内容を確認し、転記先サイト側で必要な保存操作を行ってください。</li></ol></section>
          <section class="transfer-help-grid"><article><h3>転記TSV</h3><p>キャストの転記用データをクリップボードへコピーします。保存済みのキャストで利用できます。</p><p>編集画面では、画面上で現在入力している内容をもとにTSVを生成します。</p></article><article><h3>転記BM</h3><p>転記先で実行するブックマークレットをコピーします。キャストデータそのものではありません。</p><p>コピーした文字列は、転記先の入力欄ではなく<b>ブラウザのブックマークURL欄</b>へ登録します。</p></article></section>
          <section class="transfer-help-block transfer-help-block--caution"><h3>うまく動かない場合</h3><ul><li>先に「転記TSV」を押してから、転記先でBMを実行してください。</li><li>ブックマークURLの先頭が <code>javascript:</code> のまま保存されていることを確認してください。</li><li>新規キャストで転記TSVが押せない場合は、先にキャストを保存してください。</li><li>コンボ／技能カウンターとキャスト画像は、この転記TSVの対象外です。</li><li>スマートフォンでは転記機能を使用できません。PCブラウザで実行してください。</li></ul></section>
        </div>
      </div>`;
    return dialog;
  }
