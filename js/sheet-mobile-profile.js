...
function closeDialog(){syncActiveGroupFromDialog();activeGroup=null;$("#mobile-profile-dialog")?.close();}
function bind(){document.addEventListener("click",event=>{const button=event.target.closest("[data-mobile-profile-group]");if(button)openGroup(button.dataset.mobileProfileGroup);});$("#mobile-profile-dialog-close")?.addEventListener("click",closeDialog);$("#mobile-profile-dialog")?.addEventListener("cancel",event=>{event.preventDefault();closeDialog();});$("#mobile-global-visibility")?.addEventListener("change",event=>{const original=source("visibility");if(!original||original.value===event.target.value)return;original.value=event.target.value;notifyProfileChanged();});const status=$("#mobile-save-status");if(status)new MutationObserver(renderSummaries).observe(status,{childList:true,subtree:true,attributes:true,attributeFilter:["data-state"]});}
