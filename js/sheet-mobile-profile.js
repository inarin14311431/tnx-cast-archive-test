...
function ensureStyleSheet(){if(document.querySelector('link[data-mobile-profile-style]'))return;const link=document.createElement("link");link.rel="stylesheet";link.href="./css-next/pages/sheet-mobile-profile.css?v=8";link.dataset.mobileProfileStyle="1";document.head.append(link);}
...
function basicSummaryHtml(){const rows=[["ハンドル",quote(source("handle")?.value||"")],["キャスト名",source("character_name")?.value||"名称未設定"],["ハンドルルビ",quote(source("handle_kana")?.value||"")],["キャスト名ルビ",source("character_kana")?.value||"—"],["プレイヤー",source("player_name")?.value||"—"],["所属",source("affiliation")?.value||"—"],["市民ランク",source("citizen_rank")?.value||"—"]];return `<div class="mobile-basic-summary-values">${rows.map(([label,value])=>`<span><b>${esc(label)}</b><em>${esc(value)}</em></span>`).join("")}</div>`;}
