...
function ensureStyleSheet(){if(document.querySelector('link[data-mobile-profile-style]'))return;const link=document.createElement("link");link.rel="stylesheet";link.href="./css-next/pages/sheet-mobile-profile.css?v=7";link.dataset.mobileProfileStyle="1";document.head.append(link);}
...
function basicSummaryHtml(){const handle=source("handle")?.value||"";const name=source("character_name")?.value||"名称未設定";const handleKana=source("handle_kana")?.value||"";const nameKana=source("character_kana")?.value||"—";const player=source("player_name")?.value||"—";const affiliation=source("affiliation")?.value||"—";const rank=source("citizen_rank")?.value||"—";return `<div class="mobile-basic-summary-box"><div class="mobile-basic-summary-row"><span>${esc(quote(handle))}</span><span>${esc(name)}</span></div><div class="mobile-basic-summary-row mobile-basic-summary-row--kana"><span>${esc(quote(handleKana))}</span><span>${esc(nameKana)}</span></div></div><div class="mobile-basic-summary-meta">PL ${esc(player)} / ${esc(affiliation)} / ${esc(rank)}</div>`;}
function personalSummaryHtml(){const fields=[["age","年齢"],["gender","性別"],["height","身長"],["weight","体重"],["eyes","瞳"],["hair","髪"],["skin","肌"]];return `<div class="mobile-personal-summary">${fields.map(([field,label])=>`<span><b>${label}：</b>${esc(source(field)?.value||"—")}</span>`).join("")}</div>`;}
...
function summaryText(groupKey){const group=GROUPS[groupKey];const field=group?.fields?.[0]?.[0];const value=field?source(field)?.value?.trim()||"未入力":"";return value.replace(/\s+/g," ");}
...
function bind(){document.addEventListener("click",event=>{const button=event.target.closest("[data-mobile-profile-group]");if(button)openGroup(button.dataset.mobileProfileGroup);});$("#mobile-profile-dialog-close")?.addEventListener("click",()=>{syncActiveGroupFromDialog();$("#mobile-profile-dialog")?.close();});$("#mobile-profile-dialog")?.addEventListener("input",syncActiveGroupFromDialog);$("#mobile-profile-dialog")?.addEventListener("change",syncActiveGroupFromDialog);$("#mobile-profile-dialog")?.addEventListener("cancel",event=>{event.preventDefault();syncActiveGroupFromDialog();$("#mobile-profile-dialog")?.close();});$("#mobile-global-visibility")?.addEventListener("change",event=>{const original=source("visibility");if(!original||original.value===event.target.value)return;original.value=event.target.value;notifyProfileChanged();});const status=$("#mobile-save-status");if(status)new MutationObserver(renderSummaries).observe(status,{childList:true,subtree:true,attributes:true,attributeFilter:["data-state"]});}
function init(){ensureStyleSheet();injectSummaryUi();injectDialog();injectGlobalActions();bind();setTimeout(renderSummaries,0);}init();