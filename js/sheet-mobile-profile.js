...
function lifePathSummaryHtml(){return `<div class="mobile-lifepath-summary">${LIFE_PATHS.map(([field,label,en])=>{const detail=splitLifePath(source(field)?.value);return `<div class="mobile-lifepath-row"><b>${bilingual(label,en)}</b><span>${esc(detail.name||"—")}</span><span>${esc(detail.skill||"取得技能なし")}</span></div>`;}).join("")}</div>`;}
...
function buildLifePathEditor(){return LIFE_PATHS.map(([field,label,en])=>{const detail=splitLifePath(source(field)?.value);return `<section class="mobile-lifepath-editor" data-mobile-lifepath-editor="${field}"><h3>${bilingual(label,en)}</h3><label><span class="mobile-profile-field-label">${bilingual(label,en)}</span><input data-mobile-lifepath-name value="${esc(detail.name)}"></label><label><span class="mobile-profile-field-label">${bilingual("取得技能","ACQUIRED SKILL")}</span><input data-mobile-lifepath-skill value="${esc(detail.skill)}"></label></section>`;}).join("");}
