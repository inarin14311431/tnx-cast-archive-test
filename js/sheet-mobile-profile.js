...
function buildLifePathEditor(){return LIFE_PATHS.map(([field,label,en])=>{const current=source(field)?.value||"";return `<section class="mobile-lifepath-editor" data-mobile-lifepath-editor="${field}"><h3>${bilingual(label,en)}</h3><label><span class="mobile-profile-field-label">${bilingual(label,en)}</span><input data-mobile-profile-modal-field="${field}" value="${esc(current)}"></label></section>`;}).join("");}
