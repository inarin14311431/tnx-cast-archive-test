...
async function publishDynamicShowcase(button) {
  ...
  try {
    ...
    const participantIds = [...document.querySelectorAll('#selected-casts [data-character-id][data-visibility="public"]')]
      ...
      .filter(Boolean);
    if (!participantIds.length) throw new Error("アクト紹介を公開するには、公開キャストを1名以上選択してください。");
...
async function extractShowcaseData(source) {
  ...
  const cards = [...doc.querySelectorAll(".cast-card")].map(card => {
    ...
    return {
      ...
      reading: normalizeDisplayQuotes(card.querySelector(".cast-card__reading")?.textContent?.trim() || ""),
      fullName: normalizeDisplayQuotes(card.querySelector(".cast-card__name")?.textContent?.trim() || ""),
  ...
  const trailerBody = doc.querySelector(".hero__intro")?.textContent?.trim() || "";
  return {
    version: 2,
