...
function normalizeSkillRow(row={}){const base={name:scalar(row.name),level:numberScalar(row.level),free_level:numberScalar(row.free_level),reason:Boolean(row.reason),passion:Boolean(row.passion),life:Boolean(row.life),mundane:Boolean(row.mundane),description:row.category==="style"?parseStyleDescription(row.description):text(row.description)};if(row.category!=="style")base.skill_kind=scalar(row.skill_kind);return base;}
