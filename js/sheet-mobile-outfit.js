...
function ensureCss() {
  ...
  link.href = './css-next/pages/sheet-mobile-outfit.css?v=3';
}
...
function slotField(item) {
  return `<label>部位<select data-outfit-field="slot">${optionList(SLOT_OPTIONS, item.slot || '')}</select></label>`;
}

function rangeField(item) {
  return `<label>射程<select data-outfit-field="range">${optionList(RANGE_OPTIONS, item.range || '')}</select></label>`;
}

function controlField(item) {
  return `<label>制御<select data-outfit-field="control_modifier">${controlOptions(item.control_modifier)}</select></label>`;
}
...
function open(id) {
  ...
  $('#mobile-outfit-dialog')?.showModal();
...
async function flush() {
  ...
  for (const item of outfits) {
    ...
    if (item._new) {
      const { data, error } = await supabase.from('character_outfits').insert(payload).select('*').single();
      if (error) throw error;
      Object.assign(item, data, {_new:false});
      parseConceal(item); parseDefense(item);
    } else {
      const { error } = await supabase.from('character_outfits').update(payload).eq('id', item.id).eq('character_id', character.id);
      if (error) throw error;
    }
  }
...
async function interceptSave(event) {
  ...
  try {
    ...
    const status = $('#mobile-save-status');
    if (status) { status.dataset.state = 'error'; status.textContent = `アウトフィットの保存に失敗しました：${error?.message || '不明なエラー'}`; }
  } finally {
    replaying = false;
    saving = false;
  }
...
function bind() {
  ...
  $('#mobile-outfit-close')?.addEventListener('click', () => $('#mobile-outfit-dialog')?.close());
  $('#mobile-outfit-dialog')?.addEventListener('cancel', event => { event.preventDefault(); $('#mobile-outfit-dialog')?.close(); });
  $('#mobile-outfit-fields')?.addEventListener('input', event => {
    ...
    if (control) updateActive(control);
  });
...
async function init() {
  ...
  character = data;
  const result = await supabase.from('character_outfits').select('*').eq('character_id', data.id).order('sort_order');
  if (result.error) { console.error(result.error); return; }
  outfits = (result.data || []).map(item => ({...item,_new:false}));
