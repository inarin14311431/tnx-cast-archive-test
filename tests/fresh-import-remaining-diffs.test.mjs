...
test("fresh import normalizes displayed CS, style skill prefix, and dash purchase values", () => {
  ...
  const archive = {
    character:{character_name:"スグリ",cs_base:7,cs_gear:0},
...
test("real CS changes remain detectable after equipment normalization", () => {
  const raw = {base:{name:"スグリ"},ability:{cs:"5"},armors:[{name:"HAジャケット",notes:"CS-1"}]};
  const archive = {character:{character_name:"スグリ",cs_base:7,cs_gear:0},skills:[],outfits:[{category:"armor",name:"HAジャケット",description:"CS-1",ofc_details:{}}]};
  const differences = diffCanonicalBundles(canonicalizeArchiveBundle(archive), canonicalizeCharacterSheetJsonp(raw));
