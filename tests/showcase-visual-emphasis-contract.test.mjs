...
test("visual emphasis remains the final cinematic presentation layer", () => {
  ...
  const structure = entryCss.indexOf('act-showcase-presentation-structure-pass.css?v=1');
  const emphasis = entryCss.indexOf('act-showcase-visual-emphasis.css?v=2');
  assert.ok(legibility >= 0 && scene > legibility && structure > scene && emphasis > structure);
  assert.match(entryCss.trim().split("\n").at(-1), /act-showcase-visual-emphasis\.css\?v=2/);
  assert.match(cinematicHtml, /act-showcase-entry\.css\?v=19/);
});
